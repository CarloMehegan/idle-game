...
function camera:checkBorderCollision(w,h)
  if camera.x < 0 then
    camera.x = 0
  end
  if camera.y < 0 then
  ...
  if camera.x > w - love.graphics.getWidth() then
    camera.x = w - love.graphics.getWidth()
  end
  if camera.y > h - love.graphics.getHeight() then
    camera.y = h - love.graphics.getHeight()
  end
end
